/**
 * 负数账单详情 - 主行 + 明细；单价/总价允许为负（与 penalty 类型一致）
 */
...
export default async function PenaltyBillDetailPage({ params }: PageProps) {
  ...
  if (!invoice || invoice.invoice_type !== "penalty") notFound()
    ...
    <DashboardLayout user={session.user || {}}>
      <div className="container max-w-6xl py-8">
        <InvoiceBillDetailClient
          invoiceId={id}
          invoice={serialized}
          backListHref="/dashboard/finance/bills/penalty"
          billKindLabel="负数账单"
        />
      </div>
    </DashboardLayout>
