/**
 * GET /api/wms/outbound-shipments/batch-print/bol?ids=1,2,3
 * 批量生成 BOL，返回合并后的单份 PDF（多页）
...
import { loadBatchBOLData } from '../../get-outbound-print-buffer'
...
export async function GET(request: NextRequest) {
  try {
    ...
    const buffers = await Promise.all(
      withLogo.map((d) =>
        generateBOLPDF(d).then((b) => (Buffer.isBuffer(b) ? b : Buffer.from(b as ArrayBuffer)))
      )
    )
    if (buffers.length === 0) {
      return NextResponse.json(
        { error: '没有可用的出库预约数据可生成 BOL' },
        { status: 404 }
      )
    }

    const merged = await mergePdfBuffers(buffers)
    const filename = `批量-BOL-${ids.length}份.pdf`
    ...
    console.error('[Batch BOL]', error)
    return NextResponse.json(
      { error: error?.message || '批量生成 BOL 失败' },
      { status: 500 }
    )
  }
}
