...
export function enhanceConfigWithSearchFields(config: EntityConfig): EntityConfig {
  ...
  // 如果手动定义了 filterFields，则合并：用手动定义的字段覆盖自动生成的对应字段，保留其他自动生成的字段
  const autoGeneratedFields = generateFilterFields(enhancedConfig)
  if (!enhancedConfig.list.filterFields || enhancedConfig.list.filterFields.length === 0) {
    enhancedConfig.list.filterFields = autoGeneratedFields
  } else {
    // 合并手动定义的字段和自动生成的字段
    const manualFields = enhancedConfig.list.filterFields
    const manualFieldMap = new Map(manualFields.map(f => [f.field, f]))
    const mergedFields: FilterFieldConfig[] = []
    ...
    autoGeneratedFields.forEach(field => {
      if (!manualFieldMap.has(field.field)) {
        mergedFields.push(field)
      }
    })
    
    enhancedConfig.list.filterFields = mergedFields
  }
